...
async def lifespan(app: FastAPI):
    ...
    def safe_add_column(conn, table, column, col_type, default=None):
    ...
    try:
        with engine.connect() as conn:
            # Projects table
            safe_add_column(conn, "projects", "investor", "TEXT")
            safe_add_column(conn, "projects", "total_budget", "FLOAT")
            safe_add_column(conn, "projects", "start_date", "TIMESTAMP")
            ...
            safe_add_column(conn, "projects", "map_url", "VARCHAR")
            safe_add_column(conn, "projects", "drive_url", "VARCHAR")
            safe_add_column(conn, "projects", "sheet_url", "VARCHAR")
            # Diagrams table
            safe_add_column(conn, "diagrams", "created_at", "TIMESTAMP")
            # Users table
            safe_add_column(conn, "users", "reset_token", "VARCHAR")
            safe_add_column(conn, "users", "reset_token_expire", "TIMESTAMP")
            conn.commit()
    except Exception:
        pass  # Skip migration errors silently
